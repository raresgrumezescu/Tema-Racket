...
(define (queue-empty? q)
  (if (and (equal? (queue-left q) empty-stream) (equal? (queue-right q) '()))
      #t
      #f))
...
(define (rotate left right Acc)
  (if (null? right)
      (if (stream-empty? left)
          (stream-cons right Acc)
          (stream-cons (stream-first left) (rotate (stream-rest left) right (stream-cons right Acc))))
      (if (stream-empty? left)
...
(define (enqueue x q)
  (if (>= (queue-size-l q) (+ (queue-size-r q) 1))
      ...
      (make-queue (rotate (queue-left q) (cons x (queue-right q)) empty-stream) null (+ (queue-size-l q) (queue-size-r q) 1) 0)))
...
(define (dequeue q)
  (if (>= (queue-size-l q) (+ (queue-size-r q) 1))
      ...
      (make-queue (rotate (stream-rest (queue-left q)) (queue-right q) empty-stream) null (+ (- (queue-size-l q) 1) (queue-size-r q)) 0)))
