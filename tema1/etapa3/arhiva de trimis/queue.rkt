...
(define (queue-empty? q)
  (if (and (equal? (queue-left q) '()) (equal? (queue-right q) '()))
      #t
      #f))
...
(define (enqueue x q)
  ;(define q2 (struct-copy queue q [right (cons x (queue-right q))] [size-r (+ (queue-size-r) 1)])) q2)
...
; Veți întoarce coada actualizată.
(define (put-in-right q)
  (if (null? (queue-right q))
      q
      (put-in-right (make-queue (cons (car (queue-right q)) (queue-left q)) (cdr (queue-right q)) (+ 1 (queue-size-l q)) (- (queue-size-r q) 1)))))

(define (dequeue q)
  (if (queue-empty? q)
      q
      (if (null? (queue-left q))
          (dequeue (put-in-right q))
          ;(make-queue (cdr (queue-left (put-in-right q))) (queue-right (put-in-right q)) (- (queue-size-l (put-in-right q)) 1) (queue-size-r (put-in-right q)))
