...
(define (remove-first-from-counter C)   ; testată de checker
  (cond
    ...
    ((= (+ (queue-size-r (counter-queue C)) (queue-size-l (counter-queue C))) 1) (struct-copy counter C [queue (dequeue (counter-queue C))] [tt (- (counter-tt C) (counter-et C))] [et 0]))
...
(define (append-facut-smecher l1 l2)
  (cond
    ((and (list? l1) (list? l2)) (append l1 l2))
    ((list? l1) (append l1 (cons l2 null)))
    ((list? l2) (append (cons l1 null) l2))
    (else (append (cons l1 null) (cons l2 null)))))
...
(define (cel-ce-iese C acc)
  (if (> (counter-et C) 0)
      ...
      (if (queue-empty? (counter-queue C))
          acc
          (append-facut-smecher acc (cons (counter-index C) (car (top (counter-queue C))))))))


(define (reduce-cu-x minutes)
  (λ (C)
    (if (> (counter-tt C) minutes)
        (struct-copy counter C [tt (- (counter-tt C) minutes)])
        (struct-copy counter C [tt 0]))))

(define (add-daca-exista C)
  (if (< (+ (queue-size-r (counter-queue C)) (queue-size-l (counter-queue C))) 2)
      0
      (cdr (top (dequeue (counter-queue C))))))


(define (simuleaza minutes)
  (λ (C)
    (cond
      ((> (counter-et C) minutes) (struct-copy counter C [et (- (counter-et C) minutes)]))
      ((= (counter-et C) minutes) (struct-copy counter C [et (add-daca-exista C)] [queue (dequeue (counter-queue C))]))
      ((< (counter-et C) minutes) ((simuleaza minutes) (struct-copy counter C [et (+ (counter-et C) (add-daca-exista C))] [queue (dequeue (counter-queue C))]))))))
...
(define (min-et-mai-special counters)
  (if (null? counters)
      (cons 10000 10000)
      (if (and (<= (counter-et (car counters)) (cdr (min-et-mai-special (cdr counters)))) (not (queue-empty? (counter-queue (car counters)))))
  ...
  (if (null? requests)
      ...
      (match (car requests)
        ...
        [x (serve-ajutator (cdr requests)                ;(map (pass-time-through-counter x) fast-counters)
                                  ...
                                  ;(append-facut-smecher acc (foldl cel-ce-iese null (map (pass-time-through-counter x) (append fast-counters slow-counters)))))]

                                  
                                  ;(map (simuleaza x) (map (reduce-cu-x x) fast-counters))
                                  ;(map (simuleaza x) (map (reduce-cu-x x) slow-counters))
                           ;fast-counters
                           ;slow-counters
                                  (append-facut-smecher acc (foldl cel-ce-iese null (map (pass-time-through-counter x) (append fast-counters slow-counters)))))]
                                  ;ana-are-1)]
...
;(counter 2 12 5 (make-queue '() '((geo . 7) (lia . 5)) 0 2))
